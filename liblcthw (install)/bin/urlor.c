...
{
	bstring route = (bstring)TSTree_search(routes, bdata(url), blength(url));
	if (route == NULL) {
		log_info("No exact match found, trying prefix.");
		route = (bstring)TSTree_search_prefix(routes, bdata(url), blength(url));
	}
...
bstring read_line(const char *prompt)
{
	...
	check_debug(result != NULL, "stdin is closed");
...
int main(int argc, char *argv[])
{
	...
	check(routes != NULL, "Failed to read routes file.");
