...
static inline int Hashmap_binget_node(Hashmap *map, uint32_t hash, DArray *array, void *key)
{
	int left = 0;
	int right = array->end - 1;
	HashmapNode *guess = NULL;	

	while (left <= right) {
		int mid = left + (right - left) / 2;
		guess = DArray_get(array, mid);

		int res = bstrcmp(guess->key, (bstring)key);
		if (res == 0) {
			return mid;
		}
		else if (res < 0) {
...
void *Hashmap_get(Hashmap *map, void *key, int method)
{
	...
	switch(method) {
		case 0:
			...
			break;
		case 1:
    		i = Hashmap_binget_node(map, hash, bucket, key);
    		if (i == -1) return NULL;
			break;
    }
    HashmapNode *node = DArray_get(bucket, i);
