...
struct tagbstring test4 = bsStatic("TE");

char *test_collect()
{
	Collection *coll = TSTree_collect(node, "T", strlen("T"));

	int i = 0;
	for (i = 0; i < coll->n; ++i) {
		printf("%s | ", coll->keys[i]);
	}
	printf("\n");

	collection_destroy(coll);
...
char *test_search_prefix()
{
    ...
    mu_assert(res == valueA, "Got wrong valueA for prefix of TE.");
    
    res = TSTree_search_prefix(node, "TE--", strlen("TE--"));
    mu_assert(res != NULL, "Should find for partial prefix.");
    mu_assert(res == valueA, "Got wrong valueA for prefix of TE--.");
    
    res = TSTree_search_prefix(node, "TSI", strlen("TSI"));
    mu_assert(res != NULL, "Should find for partial prefix.");
    mu_assert(res == reverse, "Got wrong reverse for prefix of TS.");
