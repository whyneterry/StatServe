...
char *test_def()
{
	uint32_t hash = Hashmap_def_hash(&test1);
	mu_assert(hash != 0, "Bad hash.");

	hash = Hashmap_def_hash(&test2);
	mu_assert(hash != 0, "Bad hash.");
	
	hash = Hashmap_def_hash(&test3);
	mu_assert(hash != 0, "Bad hash.");
...
enum { ALGO_FNV1A, ALGO_ADLER32, ALGO_DJB, ALGO_DEF };
...
char *test_distribution()
{
	...
	fill_distribution(stats[ALGO_DEF], keys, Hashmap_def_hash); 
    fprintf(stdout, "FNV\tA32\tDJB\tDEF\n");

	int fnv1sum = 0, adlersum = 0, djbsum = 0, defsum = 0;
    for (i = 0; i < BUCKETS; i++) {
        fprintf(stdout, "%d\t%d\t%d\t%d\n",
                ...
                stats[ALGO_DJB][i],
				stats[ALGO_DEF][i]);
		fnv1sum += stats[ALGO_FNV1A][i]; 
		adlersum += stats[ALGO_ADLER32][i]; 
		djbsum += stats[ALGO_DJB][i];
		defsum += stats[ALGO_DEF][i];
    }
	//printf("%d    %d    %d    %d\n", fnv1sum, adlersum, djbsum, defsum);
